...
struct EurocDataConfig : public Configurator {
    EurocDataConfig(const std::string &camera_yaml_path, const std::string &imu_yaml_path) {
        ...
        try {
            YAML::Node imu_node = YAML::LoadFile(imu_yaml_path);               // imu 外参 IMU to body
...
// 比较imu_list和image_list,比较哪个更小，返回相应类型
...
std::shared_ptr<Image> EurocDataStream::read_image() {
    ...
    img->correct_distortion(data_config->camera_intrinsic(), distortion_coeffs);   // 调用opencv函数对图像去畸变
    img->preprocess();               // 直方图均衡化
    img->t = t;
